...
# ###################################################################################################
# ########################### Chunk 4: Figure 4 (Sensitivity Plots) #################################
# ###################################################################################################
...
## For figures 5c and 5d we will first generate models 5 and 6 from models 3 and 4 by using optimized P_eff values
## obtained by substituting fperm with 0.849/1.8 (reported ka/calculated ka)
model5 <- param(model3, list(fperm=0.849/1.8))
model6 <- param(model4, list(fperm=0.849/1.8))

#Figure 5c; Model 5 with 200 mg PO 
load("../data/Fig3b_obs.Rda")  ##load observed data (digitized) from fig 3b in the ZT paper
load("../data/Fig3b_ZT.Rda")  ##load ZT predictions (digitized) from fig 3b in the ZT paper
wt <- 73
dose <- 200
...
#Figure 5d; Model 6 with 4 mg/kg PO 
load("../data/Fig4d_obs.Rda")  #load observed data (digitized) from fig 4d in the ZT paper
